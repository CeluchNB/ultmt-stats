 ...
 */
export const exportTeamStatsByGame = async (managerId = 'test', teamId = ''): Promise<void> => {
    const games: string[] = []
    ...
    for (const game of games) {
        ...
        const ws = workbook.addWorksheet(`vs ${game.teamTwo.name}`)
        ...
        const playerTotalRow = team.players.length + 3
        const lastPlayerRow = team.players.length + 2
        ...
        const teamTotalsHeaderIndex = team.players.length + 5
        const teamTotalsValueIndex = team.players.length + 6
